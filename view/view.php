...
function rtcamp_contributors_view($content) {
	...
	foreach	 ($user_ids as $user) {
		...
		if ($userdata!=FALSE) {
			$box = $box . '<div class="rtcamp_contributors">' . get_avatar($user, 44) . '&nbsp;&nbsp;<a href="' . get_author_posts_url($user) . '">' . $userdata->display_name . '</a>';
			$flag++;
			$usercount=$rtcamp_contributor_count[$user]; // gets users word count
...
function rtcamp_contributors_styles() {
	...
	//Register Style Sheet
	wp_register_style('rtcamp_contributors', plugins_url('rtcamp-assignment/view/rtcamp-style.css'));
...
function archive_contributor_role_post() {
	...
	if (strtolower(get_the_author_meta('user_login'))==$wp_query->query['author_name']) {
		echo '<div style="border-radius:15px;background-color:  #F76A4C; text-align: center; color: white;font-size:20px;"> Author </div>';
	} else {
		echo '<div style="border-radius:15px;background-color: #1AA7B2; text-align: center; color: white;font-size:20px;"> Contributor </div>';
	}
}
